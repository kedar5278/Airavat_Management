...
function Login({ ready, onLogin }: { ready: boolean; onLogin: (id: string, password: string) => Promise<void> }) {
  ...
  const hasTemplateValues = /YOUR_|example\.com|your-project|your_supabase/i.test(`${projectUrl} ${anonKey}`);
  const configured = Boolean(projectUrl && anonKey && !hasTemplateValues);
  if (!hydrated) return <div className="login-screen"><div className="login-brand"><Image src="/airavat-logo-navy.jpg" alt="Airavat Security Service" width={112} height={108} className="login-logo" /><h1>AIRAVAT</h1><div className="gold-kicker">SECURITY SERVICE</div><p>सर्वदा शक्तिशाली</p></div><div className="login-card login-hydrating" role="status">Loading secure sign-in…</div><p className="login-footer">© 2026 Airavat Security Service · Jamnagar, Gujarat</p></div>;
  return <div className="login-screen"><div className="login-brand"><Image src="/airavat-logo-navy.jpg" alt="Airavat Security Service" width={112} height={108} className="login-logo" /><h1>AIRAVAT</h1><div className="gold-kicker">SECURITY SERVICE</div><p>सर्वदा शक्तिशाली</p></div><form className="login-card" onSubmit={async e => { e.preventDefault(); setError(""); setPending(true); try { await onLogin(username, password); } catch (cause) { setError(cause instanceof Error ? cause.message : "Sign in failed."); } finally { setPending(false); } }}><div className="login-heading"><span>♙</span><div><h2>Admin Portal</h2><p>Verified sign-in · maximum two active devices</p></div></div>{!configured && <p className="setup-message">`.env.local` mein template values abhi hain ya Supabase URL/key missing hai. Apne real project URL aur anon key se replace karke dev server restart karein.</p>}<label>Admin ID<input value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" required /></label><label>Password<div className="password-box"><input type={show ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" required /><button type="button" onClick={() => setShow(!show)}>{show ? "Hide" : "Show"}</button></div></label>{error && <p className="error-text">{error}</p>}<button className="gold-button full-button" disabled={!ready || !configured || pending}>{pending ? "Verifying…" : !ready ? "Checking session…" : "Sign In to Admin Portal"}</button><p className="demo-hint">Credentials are verified securely through Supabase Auth.</p></form><p className="login-footer">© 2026 Airavat Security Service · Jamnagar, Gujarat</p></div>;
}

function PageHeading({ eyebrow, title, subtitle, action }: { eyebrow: string; title: string; subtitle: string; action?: React.ReactNode }) { return <div className="page-heading"><div><div className="eyebrow"><span>AIRAVAT SECURITY</span><i>•</i>{eyebrow}</div><h1>{title}</h1><p>{subtitle}</p></div>{action}</div>; }
...
function AttendancePage({ guards, attendance, setAttendance, date, setDate }: { guards: Guard[]; attendance: Attendance; setAttendance: (a: Attendance) => void; date: string; setDate: (d: string) => void }) {
  const [evidence,setEvidence] = useState<Array<{guard_id:string;attendance_time:string|null;latitude:number|null;longitude:number|null;selfie_path:string|null;status:string}>>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void sb.from("guard_attendance").select("guard_id,attendance_time,latitude,longitude,selfie_path,status").eq("attendance_date",date).then(async ({data,error}) => {
  ...
  return <div className="content-wrap">
    ...
    <div className="table-wrap"><table><thead><tr><th>GUARD</th><th>STATUS</th><th>TIME</th><th>LOCATION</th><th>SELFIE</th></tr></thead><tbody>{evidence.map(r => <tr key={r.guard_id}><td><strong>{guardName(r.guard_id)}</strong><small className="cell-sub">{r.guard_id}</small></td><td><span className={`status ${r.status.toLowerCase()}`}>{r.status}</span></td><td>{r.attendance_time ? new Date(r.attendance_time).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit"}) : "—"}</td><td>{r.latitude != null && r.longitude != null ? <a href={`https://www.google.com/maps?q=${r.latitude},${r.longitude}`} target="_blank" rel="noreferrer" className="text-action">Open map</a> : "—"}</td><td>{selfies[r.guard_id] ? <a href={selfies[r.guard_id]} target="_blank" rel="noreferrer"><img src={selfies[r.guard_id]} alt={guardName(r.guard_id)} style={{width:44,height:44,objectFit:"cover",borderRadius:7}} /></a> : "—"}</td></tr>)}{!evidence.length && <tr><td colSpan={5} className="empty-state">No GPS/selfie check-in has been recorded for this date.</td></tr>}</tbody></table></div>
