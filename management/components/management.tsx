...
function Login({ ready, onLogin }: { ready: boolean; onLogin: (id: string, password: string) => Promise<void> }) {
  ...
  return <div className="login-screen"><div className="login-brand"><Image src="/airavat-logo-navy.jpg" alt="Airavat Security Service" width={112} height={108} className="login-logo"/><h1>AIRAVAT</h1><div className="gold-kicker">SECURITY SERVICE</div><p>સર્વદા શક્તિશાળી</p></div>
    <div className="login-card">
      ...
      :<form onSubmit={submit}><div className="login-heading"><span>♙</span><div><h2>Guard Portal</h2><p>Sign in with your Guard ID and registered mobile number</p></div></div><label>Guard ID<input value={username} onChange={e=>setUsername(e.target.value)} autoComplete="username" required /></label><label>Registered Mobile Number<input value={password} onChange={e=>setPassword(e.target.value.replace(/\D/g,""))} inputMode="numeric" maxLength={10} autoComplete="tel" required /></label>{error&&<p className="login-error">{error}</p>}<button className="login-submit" disabled={pending}>{pending?"Signing in…":"Login as Guard"}</button></form>}
