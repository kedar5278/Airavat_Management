...
function AttendancePage({ guards, attendance, setAttendance, date, setDate }: { guards: Guard[]; attendance: Attendance; setAttendance: (a: Attendance) => void; date: string; setDate: (d: string) => void }) {
  const day = attendance[date] ?? {}; const count = (s: string) => Object.values(day).filter(x => x === s).length;
  const mark = (id: string, status: "Present" | "Absent" | "Leave") => setAttendance({ ...attendance, [date]: { ...day, [id]: status } });
  return <div className="content-wrap"><PageHeading eyebrow="Operations & deployment" title="Daily Attendance" subtitle="Record and review deployment attendance by date." action={<button className="secondary-button" onClick={() => window.print()}>▤ &nbsp; Print Sheet</button>} /><div className="attendance-date"><label className="field"><span>Attendance date</span><input type="date" value={date} onChange={e => setDate(e.target.value)} /></label><button className="secondary-button" onClick={() => setAttendance({ ...attendance, [date]: Object.fromEntries(guards.filter(g => g.status === "Active").map(g => [g.id, "Present" as const])) })}>✓ &nbsp; Mark All Present</button></div><div className="summary-row"><Metric label="PRESENT" value={count("Present")} note="On site" icon="✓" tone="green" /><Metric label="ABSENT" value={count("Absent")} note="Not on duty" icon="×" tone="red" /><Metric label="ON LEAVE" value={count("Leave")} note="Approved leave" icon="◷" tone="gold" /></div><div className="table-wrap"><table><thead><tr><th>GUARD ID</th><th>GUARD NAME</th><th>DESIGNATION</th><th>DEPLOYMENT SITE</th><th>SHIFT</th><th>ATTENDANCE</th></tr></thead><tbody>{guards.map(g => <tr key={g.id}><td><span className="id-tag">{g.id}</span></td><td><strong>{g.name}</strong><small className="cell-sub">{g.phone}</small></td><td>{g.designation}</td><td>{g.site || "—"}</td><td>{g.shift}</td><td><div className="attendance-actions">{(["Present", "Absent", "Leave"] as const).map(s => <button key={s} onClick={() => mark(g.id,s)} className={`${s.toLowerCase()} ${day[g.id] === s ? "attendance-selected" : ""}`}>{s}</button>)}</div></td></tr>)}</tbody></table></div><p className="table-foot">Attendance is saved to Supabase for {new Date(`${date}T00:00:00`).toLocaleDateString("en-IN", { dateStyle: "long" })}.</p></div>;
}
